...
class nav_cloning_node:
    def __init__(self):
        ...
        self.trajectory_path =  self.path + f"analysis/{self.pro}/trajectory_path.csv"
    ...
    def record_trajectory(self):
        # 保存ディレクトリを確認・作成
        os.makedirs(os.path.dirname(self.trajectory_path), exist_ok=True)

        # step_index を更新
        self.step_index += 1

        # CSVに追記
        file_exists = os.path.exists(self.trajectory_path)
        with open(self.trajectory_path, 'a', newline='') as fw:
            writer = csv.writer(fw)
            # ヘッダ行がまだなければ追加
            if not file_exists:
                writer.writerow(["step", "x", "y"])
            writer.writerow([self.step_index, self.gazebo_pos_x, self.gazebo_pos_y])
